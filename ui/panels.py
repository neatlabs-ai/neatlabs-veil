...
class AIChatPanel(QFrame):
    """Interactive AI analysis chat interface."""
    ...
    def __init__(self, parent=None):
        ...
        self.btn_trackers = QPushButton("🔍 TRACKER INTEL")
        ...
        self.btn_brief = QPushButton("📋 EXEC BRIEF")
        ...
        self.btn_firewall = QPushButton("🛡 BLOCK RULES")
        ...
        self.chat_display.setStyleSheet(
            ...
            "color: rgba(255,255,255,0.85); padding: 12px; "
    ...
    def _add_system_message(self, text: str):
        self.chat_display.append(
            f'<div style="color: {Colors.CYAN}; margin: 8px 0; padding: 8px; '
            f'border-left: 2px solid {Colors.CYAN}; font-family: monospace;">'
            f'<b style="color: {Colors.CYAN};">[ VEIL AI ]</b><br>'
            f'{text.replace(chr(10), "<br>")}</div>'
        )

    def _add_user_message(self, text: str):
        self.chat_display.append(
            f'<div style="color: rgba(255,255,255,0.9); margin: 8px 0; padding: 8px; '
            f'border-left: 2px solid {Colors.GREEN}; font-family: monospace;">'
            f'<b style="color: {Colors.GREEN};">[ YOU ]</b><br>{text}</div>'
        )

    def _add_ai_response(self, text: str):
        """Format and display AI response with markdown-like rendering."""
        # Simple markdown conversion
        html = text.replace("\n", "<br>")
        html = html.replace("## ", '<br><b style="color: #00f0ff; font-size: 13px;">► ')
        # Close header tags at next <br>
        import re
        html = re.sub(r'(► [^<]+)', r'\1</b>', html)
        html = html.replace("**", "<b>").replace("  - ", "<br>  • ")
        html = html.replace("- ", "• ")

        self.chat_display.append(
            f'<div style="color: rgba(255,255,255,0.85); margin: 8px 0; padding: 10px; '
            f'background: rgba(0,240,255,0.03); border-left: 2px solid {Colors.MAGENTA}; '
            f'font-family: monospace; font-size: 11px; line-height: 1.5;">'
            f'<b style="color: {Colors.MAGENTA};">[ VEIL AI — {self._ai_analyzer.model if self._ai_analyzer else "GPT-4.1-NANO"} ]</b><br>'
            f'{html}</div>'
        )
        # Scroll to bottom
    ...
    def _check_ready(self) -> bool:
        if not self._ai_analyzer or not self._ai_analyzer.is_available:
            self._add_system_message(
                ...
                "Or use the menu: AI → Set API Key"
            )
            return False
